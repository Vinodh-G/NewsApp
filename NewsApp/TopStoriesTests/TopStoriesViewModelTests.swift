...
class TopStoriesViewModelTests: XCTestCase {
    ...
    func testCollectionDataSectionCountForValidData() {
        let viewModel = mockViewModel()
        viewModel.fetchTopStories()
        XCTAssertEqual(viewModel.collectionViewData?.sections.count, 3)
    }
    ...
    func testCollectionDataSectionNewsCatTypeScience() {
        let viewModel = mockViewModel()
        viewModel.fetchTopStories()
        XCTAssertEqual((viewModel.collectionViewData?.sections[1] as? NewsSectionModel)?.categoryType, "science")
    }
    ...
    func testCollectionDataSectionNewsCatTypeOthers() {
        let viewModel = mockViewModel()
        viewModel.fetchTopStories()
        XCTAssertEqual((viewModel.collectionViewData?.sections[2] as? NewsSectionModel)?.categoryType, "others")
    }
